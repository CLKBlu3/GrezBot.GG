...
UnitType Util::GetSupplyProvider(const CCRace & race, GrezBot & bot)
{
    switch (race) 
    {
...
UnitType Util::GetTownHall(const CCRace & race, GrezBot & bot)
{
    switch (race) 
    {
...
UnitType Util::GetRefinery(const CCRace & race, GrezBot & bot)
{
    switch (race) 
    {
...
sc2::BuffID Util::GetBuffFromName(const std::string & name, GrezBot & bot)
{
...
sc2::AbilityID Util::GetAbilityFromName(const std::string & name, GrezBot & bot)
{
...
bool Util::UnitCanMetaTypeNow(const Unit & unit, const UnitType & type, GrezBot & m_bot)
{
    BOT_ASSERT(unit.isValid(), "Unit pointer was null");
